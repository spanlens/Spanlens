...
lib/org-activity.ts — org별 마지막 요청 시각 워터마크(Postgres `org_activity`). `recordOrgActivity()`(logger.ts가 CH INSERT 성공 후 호출, org당 60초 throttle) / `getOrgActivitySince()` + `orgActiveSince()` / `anyActivitySince()`. **CH를 읽는 크론은 반드시 이걸 먼저 확인**해서 신규 트래픽 없으면 ClickHouse를 아예 안 건드릴 것 (gotcha #38). 전부 fail-open — 워터마크를 못 읽으면 "활성으로 간주"라서 알림·롤업이 조용히 죽는 일 없음
...
38. **🔥 ClickHouse 비용의 진짜 드라이버는 쿼리 *양*이 아니라 쿼리 *간격* — 크론이 15분 idle 창을 계속 리셋함**: ClickHouse Cloud는 compute를 **가동 wall-clock**으로 과금하고 마지막 쿼리 뒤 **15분**이 지나야 suspend. 즉 15분보다 촘촘히 아무 쿼리나 들어오면 영원히 안 잠들고 24/7 요금이 나옴 — 데이터 양·쿼리 비용과 무관. 2026-08-18 실측: 고객 프록시 요청 하루 ~8건인데 CH 쿼리 1152건/24h, 15분 초과 공백 10번뿐, compute $8.805/일(월 $264, 청구 $273.17). 범인은 `/proxy` 핫패스가 아니라 **CH를 읽는 크론 4개**: `evaluate-alerts`(`*/15` — 단독으로 idle 창과 정확히 겹쳐 영구 가동), `check-quota-warnings`(org마다 `countMonthlyRequests` 1쿼리), `aggregate-usage`, `detect-missing-model-prices`. 게다가 **한 엔드포인트를 스케줄러 3개(vercel.json + `cron-server.yml` + Better Stack 모니터)가 각각 발사**해서(gotcha #32의 대가) 시간당 wake 창이 여러 개로 흩어짐 — vercel.json cadence만 고쳐도 Better Stack이 계속 깨움. **대응(둘 다 적용됨)**: (a) **`lib/org-activity.ts` 워터마크 게이팅이 근본 대책** — logger가 CH INSERT 성공 시 Postgres `org_activity`에 org별 마지막 요청 시각을 찍고, CH 읽는 크론은 그걸 먼저 확인해 신규 트래픽 없으면 CH 접근 0. Postgres는 어차피 깨어있으니 공짜. (b) `lib/cron-cadence.ts`의 `ranSuccessfullyWithin(job, CH_CRON_MIN_INTERVAL_MINUTES)`로 3중 발사 debounce — 유효 cadence를 스케줄러가 아니라 코드가 정함(Better Stack 대시보드 손 안 대도 됨). `evaluate-alerts`는 (a) 덕분에 debounce 없이 `*/15` 유지. **새 크론이 CH를 읽게 될 때마다** (a)를 반드시 적용하고 `src/__tests__/cron-cadence-wiring.test.ts`의 감사 목록에 등록할 것 — 이 소스 가드가 빠지면 회귀해도 행동 테스트론 안 잡힘. 참고로 2026-07~08의 `warmClickhouse` 사고(`lib/clickhouse.ts` docs)는 같은 메커니즘의 다른 얼굴 — health probe만 고치고 크론을 안 본 게 오진이었음.

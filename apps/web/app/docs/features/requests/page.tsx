      ...
      <table className="[&_th:first-child]:text-left [&_td:first-child]:text-left [&_td:first-child]:whitespace-nowrap [&_td:first-child]:align-middle">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>Gross input tokens parsed from the provider response (or streamed deltas). Includes any cached portion.</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>Sum of prompt + completion. Convenience for billing queries.</td>
          </tr>
          <tr>
            <td><code>cache_read_tokens</code></td>
            <td>
              Subset of <code>prompt_tokens</code> served from the provider&apos;s prompt cache (Anthropic / OpenAI).
              Charged at reduced rate by <a href="/docs/features/cost-tracking">cost tracking</a>. 0 for rows before 2026-05-14.
            </td>
          </tr>
          <tr>
            <td><code>cache_write_tokens</code></td>
            <td>Portion of <code>prompt_tokens</code> written to the cache (Anthropic). Billed at the cache-write premium.</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>Outgoing payload sent to the provider, up to 64KB. Authorization headers stripped before storage.</td>
          </tr>
          <tr>
            <td><code>response_body</code></td>
            <td>Incoming payload from the provider, up to 64KB. Reconstructed into non-streaming shape for SSE responses.</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td>
              Set when the call ran inside an SDK <code>observe()</code> wrapper. Groups related calls into a{' '}
              <a href="/docs/features/traces">Trace</a>.
            </td>
          </tr>
          <tr>
            <td><code>span_id</code></td>
            <td>Identifies this specific call within the trace tree.</td>
          </tr>
          <tr>

...
/**
 * Recharts bodies for the /docs figures — the SVG only, no <figure>, no
 * caption. The surrounding shell lives in ./chart-shell and is rendered by the
 * server; see ./figures for how the two halves are stitched back together.
 *
 * Data is illustrative (representative of what users will see in their own
 * /dashboard), not pulled from any live source — so these stay stable as the
 * product evolves.
 *
 * DO NOT import this module statically from a docs page.
 *
 * Two things break if you do. First, recharts (~116 KB gzip) lands in that
 * page's first-load bundle: /docs/why was shipping 1,606 KB of decoded JS,
 * most of it a chart nobody has scrolled to yet. Second, `ResponsiveContainer`
 * measures itself with ResizeObserver, which does not exist during SSR, so a
 * server render emits a 0-width SVG that the client immediately repaints at
 * the real width — a React #418 hydration mismatch (CLAUDE.md gotcha #22 D).
 * Production HTML for /docs/why contained exactly that: a `recharts-surface`
 * SVG sized against a viewport the server never had.
 *
 * Every consumer goes through ./lazy-chart-bodies, which loads this module
 * once, lazily, with `ssr: false`.
 */
...
import { COLORS, RADAR_SERIES } from './chart-shell'

/* ───────────────────────── 1. Prompt A/B: p-value + CI ───────────────────────── */

export function PromptAbChartBody() {
  const data = [
...
export function AnomalyChartBody() {
...
export function ModelPriceChartBody() {
...
export function PlanQuotaChartBody() {
...
export function FeatureCoverageRadarBody() {

...
otlpRouter.post('/v1/traces', authApiKey, requireFullScope, async (c) => {
  ...
  for (const [externalTraceId, spans] of groups) {
    try {
      ...
      //
      // Each row gets an explicit `id` rather than letting the column
      // default fill it in. `spans.id` is `DEFAULT gen_random_uuid()`, so
      // the database cannot tell the difference; assigning here keeps the
      // per-row retry below able to report which row failed.

...
import { serializeTrajectory, type TrajectorySpan } from './eval-runners/trajectory.js'
import {
  buildJudgePrompt,
  parseJudgeReply,
...
import { generateForItem, type EvalProvider } from './eval-runners/generate.js'
import {
  callJudge,
  callPairwiseJudge,
  callTrajectoryJudge,
...
} from './eval-runners/judge-calls.js'

// Re-exported so existing import sites (`from '../lib/eval-runner.js'`) keep
// working unchanged after the 4B extraction.
export { buildJudgePrompt, parseJudgeReply, buildPairwiseJudgePrompt, parsePairwiseReply, runRegex, runJsonSchema, runExactMatch, runContains, callJudge, callPairwiseJudge, callTrajectoryJudge, generateForItem }
export type {
  JudgeConfig,
  TypedScoreConfig,
  PairwiseWinner,
...
// JudgeOutcome, generateForItem, the judge transport (judgeComplete + callJudge
// + callPairwiseJudge + callTrajectoryJudge) and `pool` moved to
// ./eval-runners/{judge-calls,generate,pool}.ts in 4B and are imported +
// re-exported above. extractResponseText lives in ./eval-runners/shared.ts.
